...
T StringUtil::Trim(const T &str)
{
	...
	size_t leftFound = str.find_first_not_of(WhiteSpace(str));
	//_Tstring left(n == _Tstring::npos ? tmp : tmp.substr(n, tmp.length()));
	size_t rightFound = str.find_last_not_of(WhiteSpace(str));
...
T StringUtil::LeftTrim(const T &str)
{
	...
	T left(n == string::npos ? str : str.substr(n, str.length()));
...
T StringUtil::RightTrim(const T &str)
{
	...
	T right(n == string::npos ? str : str.substr(0, n + 1));
